...
void	init_fork(int *arr, int size)
{
	int	i;
...
void	*template(void *arg)
{
	...
	while (env->dead_found == 0 && env->cycle_end[philo_id - 1]) 
	{
		if (env->fork[philo_id - 1] == 1 && env->fork[philo_id % env->philo_size] == 1)
		{
			...
			if (dead)
			{
				usleep(get_dif_time(env->dead_arr[philo_id -1]));  // need to check dead_arr sould be the one who got minus from current gettimeofday() :TODO:
				printf("%d has dead\n", philo_id);
			}
			else
